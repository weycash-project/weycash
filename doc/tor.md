...
## 1. Run WeyCash Core behind a Tor proxy

The first step is running WeyCash Core behind a Tor proxy. This will already anonymize all
...
versions of Tor see [Section 3](#3-automatically-listen-on-tor).*
	...
	HiddenServicePort 9392 127.0.0.1:9392
	HiddenServicePort 19392 127.0.0.1:19392

The directory can be different of course, but (both) port numbers should be equal to
your weycashd's P2P listen port (9392 by default).
...
and open port 9392 on your firewall (or use -upnp).
...
WeyCash Core automatically creates a hidden service to listen on. This will positively 
affect the number of available .onion nodes.

This new feature is enabled by default if WeyCash Core is listening (`-listen`), and
...
- Do not add anything but WeyCash Core ports to the hidden service created in section 2.
