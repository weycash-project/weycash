WeyCash Core version 0.13.2 is now available from:
...
Previous versions of WeyCash Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of WeyCash Core was to raise the default minimum
relay fee.

WeyCash Core 0.13.2 will have a strict maximum size on the mempool. The
...
WeyCash Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running weycashd without having to do any manual configuration.
...
WeyCash Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. WeyCash Core will relay transactions with
...
entirely in WeyCash Core 0.13, and it is at this time undecided whether the
...
WeyCash Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
WeyCash Core automatically creates a hidden service to listen on, without
manual configuration. WeyCash Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if WeyCash Core is listening, and
...
(default: `0.0002` WEYC/kB).

At all times, WeyCash Core will cap fees at `-maxtxfee=<x>` (default:
0.10) WEYC.
Furthermore, WeyCash Core will never create transactions paying less than
...
Then, to tunnel a SSL connection on 29402 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 29402 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /monacoinrpc>
        ProxyPass http://127.0.0.1:9393/
        ProxyPassReverse http://127.0.0.1:9393/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass weycashd rpc basic auth
...
Restarting weycashd will no longer clear out the list of banned peers; instead
...
Various code modernizations have been done. The WeyCash Core code base has
...
HD wallets are incompatible with older versions of WeyCash Core.
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  note that WeyCash Core does not provide an option to use this capability as
...
Since WeyCash Core 0.10.0, nodes have defaulted to only relaying and
