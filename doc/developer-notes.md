...
- [Development guidelines](#development-guidelines)
    - [General WeyCash Core](#general-monacoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
WeyCash Core is a multi-threaded application, and deadlocks or other
...
$ valgrind -v --leak-check=full src/weycashd -printtoconsole
```
...
WeyCash Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 9392.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 9393 for connections and services them.
...
However, in open source software such as WeyCash Core, where everyone uses
...
pay attention to for reviewers of WeyCash Core code.

General WeyCash Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: WeyCash Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of WeyCash Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against WeyCash Core so that they can be integrated
...
file descriptors it will cause problems with Weycash's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Weycash's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `weycash-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `weycash-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of WeyCash Core, the wallet was always
